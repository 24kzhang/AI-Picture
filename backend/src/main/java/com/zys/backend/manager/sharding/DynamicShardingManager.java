package com.zys.backend.manager.sharding;
...
//                .eq(Space::getSpaceLevel, SpaceLevelEnum.FLAGSHIP.getValue())
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream()
                .map(tableName ->  "zys." + tableName) // 确保前缀合法
...
//        Set<String> dbNames = contextManager.getMetaDataContexts()
//                .getMetaData()
//                .getDatabases()
//                .keySet();
//        log.info("ShardingSphere databases: {}", dbNames);
    ...
    /**
     * 动态创建空间图片分表,当新建团队空间时,会自动创建对应的分表
     *
     * @param space
     */
    public void createSpacePictureTable(Space space) {
        ...
        if (space.getSpaceType() == SpaceTypeEnum.TEAM.getValue() && space.getSpaceLevel() == SpaceLevelEnum.FLAGSHIP.getValue()) {
            ...
            try {
                ...
                log.info("创建图片空间分表成功，空间 id = {}", space.getId());
                // 更新分表
                updateShardingTableNodes();
            } catch (Exception e) {
                e.printStackTrace();
